...
def get_freq_tokens_per_video(video, use_bigram, stop_words, tokenizer, stemmer):
    ...
    # Filter token with length < 3, with non english alphabet since
    # FastText is not 100% accurate and remove numerical token
    tokens_keep = []
...
def get_document_term_matrix(path_dataset, path_write_data, set_relevant_vid, word2id, use_bigram, stop_words,
    ...
    path_write_data : string
        Path to the folder where we write and keep intermediate results
    set_relevant_vid : set
        Set of the indexes of all relevant videos
    word2id : dict
        Dictionnary with token as key and its index (id_token) as value
    use_bigram : boolean
        if True, use bi_grams in the vocabulary
...
def coherence_plot(coherence_scores_cv, coherence_score_umass, n_topics_list):
    """Get the plot for coherence scores and save it

    Parameters
    ----------
    coherence_scores_cv : list
         List of the c_v coherence score for every models
    coherence_score_umass : list
        List of the u_mass coherence score for every models
    n_topics_list : list
        List of all the number of topics for computing the coherence score of
        the corresponding model
    ...
    fig.savefig('coherence_scores.pdf')


def get_coherence_scores_tune(S, spark, path_data, n_topics, alphas, betas, texts, corpus, id2word, n_jobs):
    """
        Get the cv_coherence and umass_coherence_score for the topic models
        for a defined number of topic with document concentration in "alphas" and
        topic concentration in "betas".
        ...
        n_topics : Int
            The number of topics from which the topic coherences will be computed
        alphas : list
            List of values for document concentration hyperparameter
        betas: list
            List of values for topic concentration hyperparameter
        texts : list of list
            A list of all the tokens for each video, represented as a sublist
        corpus : list of list
            Each element in the corpus correspond to a BoW vector of the data
...
def get_balanced_data_for_classifier(df_relevant_vids_classifier):
    ...
    Parameters
    ----------
    df_relevant_vids_classifier : pandas.core.frame.DataFrame
        Dataframe that contains the relevant features of the videos selected for the classifier which are the relevant
        videos that were not used for training the topic modelling model.
...
def get_data_for_classifier(id2word_final, set_relevant_vids_classifier, stop_words, tokenizer, stemmer):
    ...
    set_relevant_vids_classifier : set
        Set of the indices of all relevant videos from the dataset
    stop_words :
    tokenizer :
    stemmer :
    ...
    ----------
    S : sparse matrix
        The document-term matrix where the vocabulary correspond to the  id2word_final dictionnary
    groundtruth : list
        List of the category for each video from the document-term matrix
    """
...
def split_train_test_val(path_dataset, train_size, n_docs):
    ...
    ----------
    list_train_idx : list
        Indices of all sample for the training data
    list_test_idx : list
        Indices of all sample for the testing data
    list_val_idx : list
        Indices of all sample for the validation data
    """
...
def classifier(doc_topic_matrix, groundtruth, reg_coefs, list_train_idx, list_test_idx, list_val_idx):
    """
    Build a classifier from the document-topic matrix in order to find the category of each video

    Parameters
    ----------
    doc_topic_matrix : dense csr_matrix
        Matrix of the distribution over the topics for each document
    groundtruth : list
        List of the category for each video from the document-term matrix
    reg_coefs : list
        List of the hyperparameter of the classifier for the regularization term
    list_train_idx : list
        Indices of all sample for the training data
    list_test_idx : list
        Indices of all sample for the testing data
    ...
    ----------
    Accuracy : float
        The accuracy of a classifier for the doc_topic_matrix
    """
...
def plot_accuracy_classifier(accuracies, n_topics):
    """
    Plot the accuracy of the classifier for every model with the number of topics in n_topics

    Parameters
    ----------
    accuracies : list
        List of the accuracy of the classifier for every model with the number of topics in n_topics
    n_topics : list
        List of the number of topics of all model
    """
    fig, ax = plt.subplots(figsize=(20, 10))
    ...
    fig.savefig('accuracies_for_n_topics.pdf')


def plot_accuracy_classifier_tune(accuracies, alphas, betas):
    """
    Plot the accuracy of the classifier for every model with alpha and beta
    as hyperparameters in alphas and betas, respectively

    Parameters
    ----------
    accuracies : list
        List of the accuracy of the classifier for every model with alpha and beta
        as hyperparameters in alphas and betas, respectively
    alphas : list
            List of values for document concentration hyperparameter
    betas: list
    ...
    # fig.tight_layout()
    plt.show()
    fig.savefig('accuracies_for_alphas_betas.pdf')
